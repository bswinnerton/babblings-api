...
RSpec.describe API::V1::VimeoVideosAPI do
  describe 'GET /api/v1/vimeo-videos' do
    let!(:videos) { create_list :vimeo_video, 30  }

    before { get '/api/v1/vimeo-videos', params }
  ...
  describe 'GET /api/v1/vimeo-videos/:id' do
    let!(:video) { create :vimeo_video }

    before { get "/api/v1/vimeo-videos/#{id}" }
  ...
  describe 'POST /api/v1/vimeo-videos' do
    let(:source) { 'https://www.vimeo.com/watch?v=tIdIqbv7SPo' }
    let(:video) { create :vimeo_video }

    before do
      allow(CreateVimeoVideo).to receive(:perform).and_return video
      post '/api/v1/vimeo-videos', params
    end
    ...
    context 'with valid parameters' do
      ...
      it 'creates the video' do
        expect(CreateVimeoVideo).to have_received(:perform)
      end
    end
  ...
  describe 'DELETE /api/v1/vimeo-videos/:id' do
    let(:make_request) { delete "/api/v1/vimeo-videos/", id: video.id }
    ...
    context 'video already exists' do
      ...
      it 'deletes the video', skip_request: true do
        expect { make_request }.to change { VimeoVideo.count }.from(1).to(0)
      end
    end
